...
`examples/configs/live-bybit-ema-cross.json` trades a Bybit testnet account:
...
```json
{
  ...
  "dataClients": [
    { "factory": "BYBIT", "clientId": "BYBIT", "config": { "productType": "linear", "testnet": true,
...
Set `BYBIT_TESTNET_API_KEY` and `BYBIT_TESTNET_API_SECRET`, then:
