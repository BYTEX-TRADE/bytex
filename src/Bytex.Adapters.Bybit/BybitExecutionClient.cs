...
public sealed class BybitExecutionClient : ExecutionClientBase
{
        ...
        : base(clientId, BybitVenue.Venue, new AccountId($"{BybitVenue.Venue}-UNIFIED"), config.ProductType == BybitProductType.Spot ? AccountType.Cash : AccountType.Margin, null, OmsType.Netting, services)
    {
        ...
        _http = new BybitHttp(config, Log, requireCredentials: true, config.RecvWindowMs);
    ...
    public override async Task ConnectAsync(CancellationToken ct)
    {
        ...
        _ws = new WebSocketClient(new WebSocketClientConfig { Url = new Uri(BybitVenue.WsPrivate(_config)), PingMessage = "{\"op\":\"ping\"}", PingInterval = TimeSpan.FromSeconds(20) }, Log)
        {
    ...
    {
        long expires = DateTimeOffset.UtcNow.AddSeconds(10).ToUnixTimeMilliseconds();
    ...
    private Task HandleMessageAsync(string text)
    {
        ...
        {
            Log.LogWarning(e, "Failed to parse Bybit private message: {Text}", text.Length > 300 ? text.Substring(0, 300) : text);
        }
...
public sealed class BybitPlugin : Core.Plugins.IPlugin
{
