...
{
    /// <summary>How many messages the loop's queue holds when the caller names no capacity.</summary>
    public const int DefaultCapacity = 100_000;

    /// <summary>How long a stop waits for the queue to drain before the loop is cancelled outright.</summary>
    public static readonly TimeSpan DefaultDrainTimeout = TimeSpan.FromSeconds(10);
    ...
    public LiveKernelLoop(int capacity = DefaultCapacity, ILoggerFactory? loggerFactory = null, string threadName = "bytex-kernel")
    {
    ...
    public async Task StopAsync(TimeSpan? timeout = null)
    {
        ...
        if (await Task.WhenAny(wait, Task.Delay(timeout ?? DefaultDrainTimeout)).ConfigureAwait(false) != wait)
        {
