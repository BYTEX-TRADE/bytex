...
public readonly record struct InstrumentId : IComparable<InstrumentId>
{
    ...
    public static bool TryParse(string? value, out InstrumentId id)
    {
        ...
        string symbol = value.Substring(0, dot);
        string venue = value.Substring(dot + 1);
        if (string.IsNullOrWhiteSpace(symbol) || string.IsNullOrWhiteSpace(venue))
        {
            // A blank part is malformed text, and this method answers malformed text with false rather than an exception.
            return false;
        }
