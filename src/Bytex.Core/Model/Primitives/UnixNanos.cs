...
public readonly record struct UnixNanos(long Value) : IComparable<UnixNanos>, IComparable
{
    ...
    public static UnixNanos Parse(string iso8601)
    {
        ...
        DateTimeOffset parsed = DateTimeOffset.Parse(iso8601, CultureInfo.InvariantCulture, DateTimeStyles.AssumeUniversal | DateTimeStyles.AdjustToUniversal);
        UnixNanos result = FromDateTimeOffset(parsed);

        // Recover sub-100ns digits that DateTimeOffset parsing discards.
        ...
        if (dot >= 0)
        {
            ...
            {
                string extra = fraction.Substring(7).PadRight(2, '0').Substring(0, 2);
                result = result.AddNanos(long.Parse(extra, CultureInfo.InvariantCulture));
            }
        }
    ...
    public DateTimeOffset ToDateTimeOffset() => DateTimeOffset.UnixEpoch.AddTicks(Value / NanosPerTick);
    ...
    public override string ToString()
    {
        ...
        long subTick = Value % NanosPerTick;
