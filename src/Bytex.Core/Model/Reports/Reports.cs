...
/// <summary>
/// What a venue said when it was asked after one order: the order as the venue holds it, and that order's fills.
/// The fills are fetched with it so that a fill this node never saw is applied with the venue's own trade id,
/// quantity and commission rather than arithmetic - a reconciliation that invents a trade cannot be told apart
/// afterwards from one that happened.
/// </summary>
public sealed record QueryOrderAnswer(OrderStatusReport Report, IReadOnlyList<FillReport> Fills);
