...
public sealed class MarginAccount : Account
{
    ...
    public override void Apply(AccountState state)
    {
        ...
        // What the venue grants, when it says. A state that says nothing about leverage leaves what the account was
        // told last time, so a venue that reports it once does not have to repeat it on every balance change.
        if (state.DefaultLeverage is { } fallback)
        {
            SetDefaultLeverage(fallback);
        }

        if (state.Leverages is { } leverages)
        {
            foreach ((InstrumentId instrumentId, decimal leverage) in leverages)
            {
                SetLeverage(instrumentId, leverage);
            }
        }
    }
    ...
    public Money CalculateInitialMargin(Instrument instrument, Quantity quantity, Price price, bool useQuoteForInverse = false)
    {
        ArgumentNullException.ThrowIfNull(instrument);
        Money notional = instrument.NotionalValue(quantity, price, useQuoteForInverse);
        return new Money(notional.Amount * instrument.InitialMarginRate(Leverage(instrument.Id)), notional.Currency);
    }
    ...
    public Money CalculateMaintenanceMargin(Instrument instrument, Quantity quantity, Price price, bool useQuoteForInverse = false)
    {
        ArgumentNullException.ThrowIfNull(instrument);
        Money notional = instrument.NotionalValue(quantity, price, useQuoteForInverse);
        return new Money(notional.Amount * instrument.MaintenanceMarginRate, notional.Currency);
    }
