...
public sealed class OrderBook
{
    ...
    /// <summary>How much is resting at exactly that price on that side, or zero where nobody is quoting it.</summary>
    public decimal SizeAt(OrderSide side, decimal price) =>
        SideOf(side).TryGetValue(price, out Level? level) ? level.Size.Value : 0m;
    ...
    public void Apply(OrderBookDepth depth)
    {
        ...
        // An L1 book holds one level a side, and that level is the best one. Adding every level of a multi-level depth
        // left whichever arrived last, because each add clears the side first: a book quoting the worst of each side.
        IEnumerable<BookLevel> bids = BookType == BookType.L1 ? Best(depth.Bids, best: true) : depth.Bids;
        IEnumerable<BookLevel> asks = BookType == BookType.L1 ? Best(depth.Asks, best: false) : depth.Asks;
    ...
    public IReadOnlyList<(Price Price, Quantity Size)> SimulateFills(OrderSide side, Quantity quantity, Price? limitPrice = null)
    {
        ...
        {
            // An order for nothing fills nothing, rather than filling nothing at the best price.
            return [];
        }
    ...
    /// <summary>The one level an L1 book keeps: the highest bid or the lowest ask that carries any size.</summary>
    private static IEnumerable<BookLevel> Best(IReadOnlyList<BookLevel> levels, bool best)
    {
        BookLevel? pick = best
            ? levels.Where(l => !l.Size.IsZero).OrderByDescending(l => l.Price.Value).FirstOrDefault()
            : levels.Where(l => !l.Size.IsZero).OrderBy(l => l.Price.Value).FirstOrDefault();
        return pick is { } level ? [level] : [];
    }
