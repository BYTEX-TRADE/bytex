...
public sealed class OrderFactory
{
    ...
    public OrderFactory(TraderId traderId, StrategyId strategyId, IClock clock, int initialOrderCount = 0, int initialListCount = 0, bool useHyphens = true)
    {
        ArgumentNullException.ThrowIfNull(clock);
        TraderId = traderId;
        StrategyId = strategyId;
        _clock = clock;
        _orderIds = new ClientOrderIdGenerator(traderId, strategyId, clock, initialOrderCount, useHyphens);
        _listIds = new OrderListIdGenerator(traderId, strategyId, clock, initialListCount);
    }
