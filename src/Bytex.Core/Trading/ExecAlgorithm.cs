...
/// <summary>
/// A strategy that cannot do what it was written to do unless certain execution algorithms are running - a strategy
/// document whose order nodes ask to be worked in slices, for one: the order it sends names an algorithm, and an order
/// naming an algorithm nobody registered is denied.
/// <para>
/// The trader registers what is asked for when the strategy is added, and a host's own algorithm under the same id
/// wins whichever order the two were added in: what the strategy asked for is the id, not this instance of it.
/// </para>
/// </summary>
public interface INeedsExecAlgorithms
{
    /// <summary>
    /// The algorithms this strategy's orders name, built ready to register. Called once, when the strategy is added
    /// to a trader.
    /// </summary>
    IEnumerable<ExecAlgorithm> RequiredExecAlgorithms();
}
