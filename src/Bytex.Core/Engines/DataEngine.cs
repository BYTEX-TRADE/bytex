...
public sealed class DataEngine : Component, IDataClientSink
{
    ...
    /// <summary>
    /// <paramref name="post"/> runs an action on the kernel thread; null runs it inline, which is what a backtest wants.
    /// </summary>
    public DataEngine(IMessageBus bus, Cache cache, DataEngineConfig? config = null, Action<Action>? post = null)
    ...
    private void Dispatch(Task task, string description, Action<string>? onFailure = null)
    {
        ...
        // A request that fails has an actor waiting for it. Logging the failure and answering nothing left that actor
        // waiting for good, so the failure is delivered as the answer, on the kernel thread like every other message.
