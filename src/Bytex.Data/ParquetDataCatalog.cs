...
public sealed class ParquetDataCatalog
{
    ...
    public Task WriteFundingRatesAsync(IEnumerable<FundingRateUpdate> rates, CancellationToken ct = default) =>
        WriteGroupedAsync(rates, r => r.InstrumentId.Value, r => r.TsInit, FundingDir, FundingRow.From, ct);
    ...
    public Task<IReadOnlyList<FundingRateUpdate>> FundingRatesAsync(InstrumentId instrumentId, UnixNanos? start = null, UnixNanos? end = null, CancellationToken ct = default) =>
        ReadAsync<FundingRow, FundingRateUpdate>(FundingDir, instrumentId.Value, start, end, r => r.ToUpdate(instrumentId), f => f.TsInit, ct);
    ...
    public IReadOnlyList<CatalogEntry> Entries()
    {
        ...
        foreach ((string kind, string label) in new[] { (QuotesDir, "quotes"), (TradesDir, "trades"), (BarsDir, "bars"), (DeltasDir, "book_deltas"), (FundingDir, "funding") })
        {
    ...
    // could not be mapped back, and EUR/USD.SIM was listed as, and shared a directory with, EUR_USD.SIM.
    private static string SafeName(string key) => key
    ...
    public sealed class BarRow
    {
        ...
        public Bar ToBar(BarType barType) => new(barType, new Price(Open, PricePrecision), new Price(High, PricePrecision), new Price(Low, PricePrecision), new Price(Close, PricePrecision), new Quantity(Volume, SizePrecision), new UnixNanos(TsEvent), new UnixNanos(TsInit), IsRevision);
    }
    ...
    public sealed class FundingRow
    {
        ...
        {
            TsEvent = update.TsEvent.Value,
            TsInit = update.TsInit.Value,
            Rate = update.Rate,
            NextFundingTime = update.NextFundingTime?.Value ?? 0L,
        };

        public FundingRateUpdate ToUpdate(InstrumentId instrumentId) =>
            new(instrumentId, Rate, NextFundingTime == 0L ? null : new UnixNanos(NextFundingTime), new UnixNanos(TsEvent), new UnixNanos(TsInit));
    }
