...
{
    [Fact]
    public void An_identifier_works_as_a_dictionary_key()
    {
        // The defect this pins: the converters handled an identifier as a VALUE and not as a property name, so any
        // option keyed by one refused to deserialise with "InstrumentId is not a supported dictionary key" - and the
        // shipped live example carried maxNotionalPerOrder, so the only live example crashed as shipped.
        Dictionary<InstrumentId, decimal> byInstrument = new()
        {
            [InstrumentId.Parse("BTCUSDT-PERP.BYBIT")] = 5_000m,
            [InstrumentId.Parse("ETHUSDT.BINANCE")] = 250.5m,
        };

        string json = BytexJson.Serialize(byInstrument);
        Dictionary<InstrumentId, decimal>? back = BytexJson.Deserialize<Dictionary<InstrumentId, decimal>>(json);

        Assert.Contains("BTCUSDT-PERP.BYBIT", json, StringComparison.Ordinal);
        Assert.Equal(byInstrument, back);
    }

    [Fact]
    public void Every_string_identifier_works_as_a_dictionary_key()
    {
        // The same hole existed for every identifier the factory covers, so a configuration keyed by a strategy or a
        // venue would have failed the same way the instrument-keyed one did.
        Dictionary<StrategyId, int> byStrategy = new() { [new StrategyId("S-001")] = 1 };
        Dictionary<Venue, string> byVenue = new() { [new Venue("BYBIT")] = "linear" };

        Assert.Equal(byStrategy, BytexJson.Deserialize<Dictionary<StrategyId, int>>(BytexJson.Serialize(byStrategy)));
        Assert.Equal(byVenue, BytexJson.Deserialize<Dictionary<Venue, string>>(BytexJson.Serialize(byVenue)));
    }
