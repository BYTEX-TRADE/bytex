...
public class OrderConstructionTests
{
    ...
    [Fact]
    public void A_market_order_still_refuses_gtd_for_its_own_reason()
    {
        // The rule about an expiry lives in the base class now; a market order does not rest at all, and says so.
        OrderParams gtd = BtcParams() with { TimeInForce = TimeInForce.Gtd };

        ArgumentException refused = Assert.Throws<ArgumentException>(() => MarketOrder.Create(gtd));
        Assert.Contains("do not support time in force", refused.Message, StringComparison.Ordinal);
    }
    ...
    [InlineData(OrderType.StopLimit)]
    [InlineData(OrderType.TrailingStopMarket)]
    public void A_resting_order_with_an_expiry_is_accepted_and_keeps_it(OrderType type)
    {
        UnixNanos expiry = new(1_700_000_000_000_000_000L);
        OrderParams gtd = BtcParams() with { TimeInForce = TimeInForce.Gtd };

        Order order = Create(type, gtd, expireTime: expiry);
