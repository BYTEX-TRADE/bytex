...
public class AccountTests
{
    ...
    [Fact]
    public void Margins_are_the_instrument_rate_on_notional_divided_by_leverage()
    {
        MarginAccount account = UsdtMargin();
        CryptoPerpetual perp = EthPerp();
        account.SetLeverage(perp.Id, 10m);

        // Notional 2 * 2000 = 4000; / 10 leverage = 400; 5% initial = 20, 2.5% maintenance = 10.
        Assert.Equal(Usdt("20"), account.CalculateInitialMargin(perp, Quantity.Parse("2.000"), Price.Parse("2000.00")));
        Assert.Equal(Usdt("10"), account.CalculateMaintenanceMargin(perp, Quantity.Parse("2.000"), Price.Parse("2000.00")));
    }
    ...
    public void Inverse_margins_are_in_base_currency()
    {
        ...
        // Notional 100 000 / 50 000 = 2 BTC; / 20 = 0.1; 1% initial = 0.001 BTC.
        Assert.Equal(new Money(0.001m, Currencies.BTC), account.CalculateInitialMargin(inverse, Quantity.Parse("100000"), Price.Parse("50000.0")));
    }
