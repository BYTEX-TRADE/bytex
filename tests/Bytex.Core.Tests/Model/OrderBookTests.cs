...
public class OrderBookTests
{
    ...
    [Fact]
    public void An_L1_book_keeps_the_best_level_whatever_order_the_depth_arrives_in()
    {
        OrderBook book = new(_btc, BookType.L1);
        OrderBookDepth worstFirst = new(
            _btc,
            [new BookLevel(Price.Parse("98.00"), Quantity.Parse("2")), new BookLevel(Price.Parse("100.00"), Quantity.Parse("1"))],
            [new BookLevel(Price.Parse("102.00"), Quantity.Parse("4")), new BookLevel(Price.Parse("100.50"), Quantity.Parse("3"))],
            RecordFlags.Snapshot,
            1,
            At(1),
            At(1));

        book.Apply(worstFirst);
    ...
    [Fact]
    public void An_L1_book_ignores_a_level_with_no_size_when_it_picks_the_best_one()
    {
        OrderBook book = new(_btc, BookType.L1);
        OrderBookDepth depth = new(
            _btc,
            [new BookLevel(Price.Parse("101.00"), Quantity.Parse("0")), new BookLevel(Price.Parse("100.00"), Quantity.Parse("1"))],
            [new BookLevel(Price.Parse("100.50"), Quantity.Parse("3"))],
            RecordFlags.Snapshot,
            1,
            At(1),
            At(1));
        ...
        // 101.00 is the highest price but nothing is offered there, so it is not a bid.
        Assert.Equal(Price.Parse("100.00"), book.BestBidPrice);
        Assert.Equal(Price.Parse("100.50"), book.BestAskPrice);
    }

    [Fact]
    public void An_L2_book_still_keeps_every_level_of_a_depth()
    {
        OrderBook book = new(_btc, BookType.L2);
        OrderBookDepth depth = new(
            _btc,
        ...
        Assert.Equal(2, book.BidLevelCount);
        Assert.Equal(2, book.AskLevelCount);
        Assert.Equal(Price.Parse("100.00"), book.BestBidPrice);
    }

    [Fact]
    public void Simulated_fills_for_a_zero_quantity_touch_no_level_on_either_side()
    {
        OrderBook book = TwoLevelBook();

        Assert.Empty(book.SimulateFills(OrderSide.Buy, Quantity.Parse("0")));
        Assert.Empty(book.SimulateFills(OrderSide.Sell, Quantity.Parse("0")));
        Assert.Empty(book.SimulateFills(OrderSide.Buy, Quantity.Parse("0"), Price.Parse("200.00")));
    }
}
