...
{
    public KernelHarness(RiskEngineConfig? risk = null, OmsType clientOms = OmsType.Netting, TradingEnvironment environment = TradingEnvironment.Backtest)
    {
        Clock = new TestClock(TestOrders.T0);
        Kernel = new CoreKernel(new KernelConfig { TraderId = TestIds.Trader, RiskEngine = risk ?? new RiskEngineConfig(), InstanceId = "test", Environment = environment }, Clock);
