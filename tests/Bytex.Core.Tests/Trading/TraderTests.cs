...
public class TraderTests
{
    ...
    /// </summary>
    private sealed class NeedyStrategy : ProbeStrategy, INeedsExecAlgorithms
    {
        private readonly Func<ExecAlgorithm> _algorithm;

        public NeedyStrategy(string id, Func<ExecAlgorithm> algorithm)
    ...
    private static TwapExecAlgorithm NewTwap() => new();
    ...
    {
        // Nothing the host has to know: the strategy says what its orders name, and the trader has it running before
        ...
        ExecAlgorithm registered = Assert.Single(h.Kernel.Trader.ExecAlgorithms);
        Assert.Equal(TwapExecAlgorithm.DefaultExecAlgorithmId, registered.ExecAlgorithmId);
        Assert.True(registered.IsRegistered);
        Assert.Equal(1, strategy.Asked);
        Assert.Same(registered, h.Kernel.Trader.Actor(new ActorId(TwapExecAlgorithm.DefaultExecAlgorithmId.Value)));
    }
    ...
    public void Two_strategies_asking_for_the_same_algorithm_get_the_one_that_is_already_running()
    {
        ...
        h.Kernel.Trader.AddStrategy(new NeedyStrategy("S-001", NewTwap));
        h.Kernel.Trader.AddStrategy(new NeedyStrategy("S-002", NewTwap));

        // One algorithm, not two under one id and not an exception: the second strategy asked for the id, and the id
        // is running.
        Assert.Single(h.Kernel.Trader.ExecAlgorithms);
    }
    ...
    public void The_hosts_own_algorithm_runs_instead_of_the_one_a_strategy_asked_for_whichever_came_first()
    {
        ...
        {
            using KernelHarness h = new();
            TwapExecAlgorithm mine = new(new TwapExecAlgorithmConfig { Horizon = TimeSpan.FromHours(1), Interval = TimeSpan.FromMinutes(10) });
            NeedyStrategy strategy = new("S-001", NewTwap);
            if (hostFirst)
            {
                h.Kernel.Trader.AddExecAlgorithm(mine);
                h.Kernel.Trader.AddStrategy(strategy);
            }
            else
            {
            ...
            Assert.Same(mine, Assert.Single(h.Kernel.Trader.ExecAlgorithms));
            Assert.Equal(TimeSpan.FromHours(1), ((TwapExecAlgorithm)Assert.Single(h.Kernel.Trader.ExecAlgorithms)).Config.Horizon);
            Assert.Same(mine, h.Kernel.Trader.Actor(new ActorId(TwapExecAlgorithm.DefaultExecAlgorithmId.Value)));
        }
    }
    ...
    public void A_twap_answers_to_the_name_a_document_calls_it_by()
    {
        ...
        Assert.Equal("TWAP", TwapExecAlgorithm.DefaultExecAlgorithmId.Value);
        Assert.Equal(TwapExecAlgorithm.DefaultExecAlgorithmId, new TwapExecAlgorithm().ExecAlgorithmId);
        Assert.Equal(new ExecAlgorithmId("MyTwap"), new TwapExecAlgorithm(new TwapExecAlgorithmConfig { ExecAlgorithmId = new ExecAlgorithmId("MyTwap") }).ExecAlgorithmId);
    }
