...
public class StrategyTests
{
    ...
    [Fact]
    public void What_the_kernel_runs_in_is_told_to_everything_it_registers()
    {
        using KernelHarness h = new(environment: TradingEnvironment.Sandbox);
        ProbeStrategy strategy = h.StartWithStrategy();

        Assert.Equal(TradingEnvironment.Sandbox, strategy.RunningIn);
    }

    [Fact]
    public void A_strategy_nobody_told_assumes_a_backtest()
    {
        // The safe default for anything constructed outside a kernel: a strategy that has not been registered has
        // not been told, and assuming live there would be the dangerous way round to be wrong.
    ...
    [Fact]
    public void Order_list_submission_caches_the_whole_list_and_sends_only_what_the_venue_should_hold_now()
    {
        // A bracket is one list of three orders and the strategy owns all three from the moment it submits, but only
        // the entry belongs at the venue: its stop and target are one-triggers-other children and protect a position
