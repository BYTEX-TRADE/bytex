...
public class PluginRegistryTests
{
    ...
    [Fact(Skip = "BUG: Plugins.cs:312-329 MergeBaseConfig copies every non-null base property, so base-config defaults (false, Unspecified, []) overwrite values the JSON payload set, contrary to its own comment 'where the typed config left defaults'")]
