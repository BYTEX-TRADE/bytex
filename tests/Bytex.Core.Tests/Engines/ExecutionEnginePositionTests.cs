...
public class ExecutionEnginePositionTests
{
    ...
    [Fact]
    public void Netting_redelivered_flipping_fill_publishes_nothing_more()
    {
        ExecHarness h = new();
        h.FillMarket("O-1", OrderSide.Buy, "1.000", "50000.00");
        MarketOrder flipping = h.SubmitAndAccept(TestOrders.Market("O-2", TestIds.BtcUsdt, OrderSide.Sell, "1.500"));
        OrderFilled fill = TestEvents.Filled(flipping, "T-2", "1.500", "52000.00", commission: "3");
        h.Engine.Process(fill);
        h.OrderEvents.Messages.Clear();
        h.PositionEvents.Messages.Clear();
        ...
        Assert.Empty(h.PositionEvents.Messages);
        Assert.Equal(Quantity.Parse("1.500"), flipping.FilledQuantity);
        // (52,000 - 50,000) * 1 - 2 commission = 1,998, booked once.
        Assert.Equal(Money.Parse("1998 USDT"), Assert.Single(h.Cache.PositionsClosed()).RealizedPnl);
    }

    [Fact]
    public void Netting_a_second_trade_after_a_flip_is_applied_although_the_flip_was_split()
    {
        ExecHarness h = new();
        h.FillMarket("O-1", OrderSide.Buy, "1.000", "50000.00");
        MarketOrder flipping = h.SubmitAndAccept(TestOrders.Market("O-2", TestIds.BtcUsdt, OrderSide.Sell, "2.000"));
        h.Engine.Process(TestEvents.Filled(flipping, "T-2", "1.500", "52000.00"));

        h.Engine.Process(TestEvents.Filled(flipping, "T-3", "0.500", "52000.00"));

        // Long 1.000, sold 1.500 then 0.500: short 1.000.
    ...
    [Fact]
    public void Hedging_order_submitted_against_a_position_id_reduces_that_position_and_opens_no_other()
    {
        ExecHarness h = new(clientOms: OmsType.Hedging);
        h.FillMarket("O-1", OrderSide.Buy, "1.000", "50000.00");
        h.FillMarket("O-2", OrderSide.Buy, "2.000", "50000.00");
        PositionId target = h.Cache.PositionsOpen().Single(p => p.Quantity == Quantity.Parse("2.000")).Id;

        h.FillMarket("O-3", OrderSide.Sell, "0.500", "51000.00", commandPositionId: target);

        Assert.Equal(2, h.Cache.PositionsOpenCount());
        Assert.Equal(Quantity.Parse("1.500"), h.Cache.Position(target)!.Quantity);
        // (51,000 - 50,000) * 0.5 = 500
        Assert.Equal(Money.Parse("500 USDT"), h.Cache.Position(target)!.RealizedPnl);
    }

    [Fact]
    public void Hedging_order_without_a_position_id_still_opens_its_own_position()
    {
        ExecHarness h = new(clientOms: OmsType.Hedging);
        h.FillMarket("O-1", OrderSide.Buy, "1.000", "50000.00");

        h.FillMarket("O-2", OrderSide.Sell, "1.000", "51000.00");
