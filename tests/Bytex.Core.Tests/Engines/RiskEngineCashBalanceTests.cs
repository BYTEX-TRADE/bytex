...
public class RiskEngineCashBalanceTests
{
    ...
    {
        // A spot instrument asks no initial margin of its own, so on a margin account at 20x leverage an order needs
        // a twentieth of its notional rather than all of it: 2,500 of a 50,000 order, which this balance covers.
        // What is being pinned is that the cash-balance rule does not also apply - it would have refused this for
        // want of 50,000.
        RiskHarness h = new();
        h.Cache.AddInstrument(TestInstruments.BtcUsdt());
        MarginAccount account = new(TestEvents.MarginState(TestIds.BinanceAccount, (Currencies.USDT, 3_000m, 0m)));
        account.SetDefaultLeverage(20m);
