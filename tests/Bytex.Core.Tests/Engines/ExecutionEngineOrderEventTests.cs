...
public class ExecutionEngineOrderEventTests
{
    ...
    [Fact]
    public void Duplicate_fill_leaves_the_order_and_the_position_as_they_were()
    {
        ExecHarness h = new();
        LimitOrder order = h.SubmitAndAccept(TestOrders.Limit("O-1", TestIds.BtcUsdt, OrderSide.Buy, "1.000", "50000.00"));
        OrderFilled fill = TestEvents.Filled(order, "T-1", "0.400", "50000.00", commission: "1");
        h.Engine.Process(fill);

        h.Engine.Process(fill with { EventId = Guid.NewGuid() });
        h.Engine.Process(TestEvents.Filled(order, "T-2", "0.600", "50000.00", commission: "1"));

        Assert.Equal(OrderStatus.Filled, order.Status);
        Assert.Equal(Quantity.Parse("1.000"), order.FilledQuantity);
        Position position = Assert.Single(h.Cache.PositionsOpen());
        Assert.Equal(1.000m, position.SignedQuantity);
        Assert.Equal(Money.Parse("-2 USDT"), position.RealizedPnl);
        Assert.Equal(2, h.OrderEvents.Of<OrderFilled>().Count);
    }
