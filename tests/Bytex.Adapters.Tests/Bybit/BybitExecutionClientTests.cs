...
public sealed class BybitExecutionClientTests
{
    ...
    [Fact(Skip = "BUG: HandleExecution turns every execution-topic row into an OrderFilled; Bybit also pushes non-trade rows there (execType Funding, with execQty = position size), which would be booked as phantom fills every funding interval")]
