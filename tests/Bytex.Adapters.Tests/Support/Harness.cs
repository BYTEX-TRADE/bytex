...
internal sealed class TestKernel : IDisposable
{
    ...
    public TestKernel(Microsoft.Extensions.Logging.ILoggerFactory? logs = null)
    {
        Clock = new TestClock(Now);
        Kernel = new Kernel(new KernelConfig { Environment = TradingEnvironment.Live, TraderId = new TraderId("TESTER-001") }, Clock, logs);
    }
...
internal static class Commands
{
    ...
    /// <summary>
    /// The commands a client really implements, for comparing two clients of one venue against each other. The
    /// adapter parity table finds a venue's clients by naming convention, so a venue with two clients of one kind -
    /// KuCoin, since its futures market arrived - has one the table cannot see.
    /// </summary>
    public static string[] Overridden(Type client) =>
        [.. client.GetMethods(System.Reflection.BindingFlags.Public | System.Reflection.BindingFlags.Instance)
            .Where(m => m.DeclaringType == client && m.Name.EndsWith("Async", StringComparison.Ordinal))
            .Select(m => m.Name)
    ...
    public static RequestFundingRates RequestFundingRates(InstrumentId instrumentId, UnixNanos? start, UnixNanos? end, int? limit) => new(instrumentId, start, end, limit, null, Guid.NewGuid(), TestKernel.Now);
