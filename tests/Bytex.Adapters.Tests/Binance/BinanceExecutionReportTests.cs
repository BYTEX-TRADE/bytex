...
public sealed class BinanceExecutionReportTests
{
    ...
    public async Task Asking_after_one_order_reaches_the_venue()
    {
        ...
        rig.Routes
            .On("GET", rig.OrderPath, OrderRow(status: "FILLED", executed: "1.00000000", cumQuote: "25000.10000000"))
            .On("GET", "/api/v3/myTrades", """
                [{"symbol":"BTCUSDT","id":28457,"orderId":4293153,"orderListId":-1,"price":"25000.10000000","qty":"1.00000000","quoteQty":"25000.10","commission":"0.00100000","commissionAsset":"BTC","time":1499865549590,"isBuyer":false,"isMaker":true,"isBestMatch":true}]
                """);

        await rig.Client.QueryOrderAsync(
            new QueryOrder(rig.Kernel.Services.TraderId, BinanceExecRig.Strategy, rig.Instrument.Id, new ClientOrderId("O-20231114-001"), null, null, Guid.NewGuid(), TestKernel.Now),
            CancellationToken.None).WaitAsync(Wait.Timeout);

        RecordedRequest order = Assert.Single(rig.Server.RequestsTo(rig.OrderPath));
        BinanceExecRig.AssertParameters(order, "symbol=BTCUSDT;origClientOrderId=O-20231114-001", "orderId");
        BinanceExecRig.AssertSigned(order);

        // And that order's fills, by the id the venue gave it, so a fill this node missed is applied with the
        // venue's own trade id rather than worked out by subtraction.
        RecordedRequest trades = Assert.Single(rig.Server.RequestsTo("/api/v3/myTrades"));
        Assert.Equal("4293153", trades.Query("orderId"));
        BinanceExecRig.AssertSigned(trades);
    }
