...
// Why: a wrong base URL sends futures orders to the spot API (or live orders to production instead of testnet),
...
{
    [Theory]
    [InlineData(BinanceAccountType.Spot, false, "https://api.binance.com")]
    [InlineData(BinanceAccountType.Spot, true, "https://testnet.binance.vision")]
    [InlineData(BinanceAccountType.UsdMFutures, false, "https://fapi.binance.com")]
    [InlineData(BinanceAccountType.UsdMFutures, true, "https://testnet.binancefuture.com")]
    public void Rest_base_url_depends_on_account_type_and_testnet(BinanceAccountType type, bool testnet, string expected)
    {
        BinanceDataClientConfig config = new() { AccountType = type, Testnet = testnet };

        Assert.Equal(expected, BinanceVenue.HttpBase(config));
    }

    [Theory]
    [InlineData(BinanceAccountType.Spot, false, "wss://stream.binance.com:9443")]
    [InlineData(BinanceAccountType.Spot, true, "wss://stream.testnet.binance.vision")]
    [InlineData(BinanceAccountType.UsdMFutures, false, "wss://fstream.binance.com")]
    [InlineData(BinanceAccountType.UsdMFutures, true, "wss://stream.binancefuture.com")]
    public void Stream_base_url_depends_on_account_type_and_testnet(BinanceAccountType type, bool testnet, string expected)
    {
        BinanceExecutionClientConfig config = new() { AccountType = type, Testnet = testnet };
    ...
    {
        BinanceDataClientConfig config = new() { AccountType = BinanceAccountType.UsdMFutures, Testnet = true, BaseUrlHttp = "http://127.0.0.1:9", BaseUrlWs = "ws://127.0.0.1:9" };
