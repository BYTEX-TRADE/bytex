...
public sealed class TardisDataClientTests : IDisposable
{
    ...
    [Fact]
    public async Task A_live_subscription_is_refused_outright_so_there_is_never_one_to_undo()
    {
        // The parity table says unsubscribing from this client is the base's do-nothing, and that it is correct here
        // rather than a gap. This is the reason, and it is only true while subscribing is refused: the moment this
        // client can hold a subscription, doing nothing to end one stops being right and the row has to change.
        ...
        SubscribeTradeTicks subscribe = new(_perpId, null, Guid.NewGuid(), TestKernel.Now);

        await client.SubscribeAsync(subscribe, CancellationToken.None).WaitAsync(Wait.Timeout);

        (SubscribeCommand refused, string reason) = Assert.Single(sink.SubscriptionFailures);
        Assert.Same(subscribe, refused);
        Assert.Contains("historical data only", reason, StringComparison.Ordinal);
        Assert.Empty(server.Requests);

        // And so unsubscribing asks nothing of anybody, which is the whole of the correct behaviour.
        await client.UnsubscribeAsync(new UnsubscribeTradeTicks(_perpId, null, Guid.NewGuid(), TestKernel.Now), CancellationToken.None).WaitAsync(Wait.Timeout);
