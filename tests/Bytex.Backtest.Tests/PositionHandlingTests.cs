...
public sealed class PositionHandlingTests
{
    ...
    [Fact(Skip = "BUG: under HEDGING the position id given to SubmitOrder is ignored when the fill is applied, so a closing order opens a new opposite position instead")]
    ...
    [Fact(Skip = "BUG: the venue's reduce-only check uses the net position, so with hedged long and short legs (net 0) the default reduce-only close of either leg is rejected")]
