...
public sealed class MarketOrderMatchingTests
{
    ...
    [Fact]
    public void With_trade_only_data_both_sides_of_the_book_follow_the_last_print()
    {
        using SimHarness sim = SimHarness.Spot();
        MarketOrder? afterLow = null;
        MarketOrder? afterHigh = null;
        sim.Trade(1000, 100.00m)
            .Trade(2000, 90.00m)
            .At(2500, s => afterLow = s.Submit(s.Orders.Market(sim.Id, OrderSide.Buy, sim.Qty(1m))))
            .Trade(3000, 110.00m)
            .At(3500, s => afterHigh = s.Submit(s.Orders.Market(sim.Id, OrderSide.Sell, sim.Qty(1m))))
            .Trade(4000, 110.00m)
            .Run();

        // The buy pays the low print it was submitted after, not the 100.00 the run opened at; the sell receives the
        // high print, not the 90.00 the bid had sunk to.
        Assert.Equal(sim.Px(90.00m), sim.SingleFill(afterLow!).LastPx);
        Assert.Equal(sim.Px(110.00m), sim.SingleFill(afterHigh!).LastPx);
    }

    [Fact]
    public void A_quote_after_a_print_takes_the_book_back_from_it()
    {
        using SimHarness sim = SimHarness.Spot();
        MarketOrder? order = null;
        sim.Quote(1000, 100.00m, 100.10m)
            .Trade(1500, 100.70m)             // drags the ask to 100.70
            .Quote(2000, 100.00m, 100.10m)    // the venue quotes again: the drag is over
            .At(2500, s => order = s.Submit(s.Orders.Market(sim.Id, OrderSide.Buy, sim.Qty(1m))))
            .Quote(3000, 100.00m, 100.10m)
            .Run();
