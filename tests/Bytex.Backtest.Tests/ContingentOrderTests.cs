...
public sealed class ContingentOrderTests
{
    ...
    [Fact]
    public void A_dormant_exit_keeps_the_venue_order_id_it_was_given_when_it_starts_working()
    {
        using SimHarness sim = SimHarness.Spot();
        OrderList? bracket = null;
        sim.Quote(1000, 100.00m, 100.10m)
            .At(1500, s => bracket = s.SubmitList(s.Orders.BracketOrder(sim.Id, OrderSide.Buy, sim.Qty(1m), sim.Px(95.00m), sim.Px(101.00m), entryPrice: sim.Px(99.00m), entryType: OrderType.Limit)))
            .Quote(3000, 98.80m, 98.90m) // the entry fills, so the exits start working
            .Quote(4000, 98.80m, 98.90m)
            .Run();

        Order stopLoss = bracket!.Orders[1];
        Order takeProfit = bracket.Orders[2];

        // One acceptance each, and - the part that mattered - the venue holds the same venue order id its owner was
        // given. Working the exit as a fresh order gave it a second id at the venue that its owner never heard of, so a
        // cancel or a status report by venue id would have been about an order neither side could name.
        Assert.Single(sim.Events.OfType<OrderAccepted>(), e => e.ClientOrderId == stopLoss.ClientOrderId);
        Assert.Single(sim.Events.OfType<OrderAccepted>(), e => e.ClientOrderId == takeProfit.ClientOrderId);
        Assert.Equal(OrderStatus.Accepted, stopLoss.Status);
        Assert.Equal(2, sim.Exchange.OpenOrderCount);

        var atVenue = sim.Exchange.GenerateMassStatus().OrderReports;
        Assert.Equal(stopLoss.VenueOrderId, Assert.Single(atVenue, r => r.ClientOrderId == stopLoss.ClientOrderId).VenueOrderId);
        Assert.Equal(takeProfit.VenueOrderId, Assert.Single(atVenue, r => r.ClientOrderId == takeProfit.ClientOrderId).VenueOrderId);
    }
}
