...
public sealed class ReportsAndSerializationTests
{
    ...
    [Fact]
    public void Account_table_tracks_the_balance_after_the_initial_state_and_after_every_fill()
    {
        using SimHarness sim = StatisticsScenario.Run();
        ...
        Assert.Equal(
            [1_000_000m, 999_999m, 1_000_008m, 1_000_007m, 1_000_000m, 999_999m, 1_000_002m, 1_000_001m],
    ...
    public void Json_document_has_a_stable_shape_with_camel_case_names()
    {
        ...
        Assert.Equal(
            [
                ...
                "totalOrders", "totalPositions", "currencies", "trades", "equityCurves", "orders", "fills", "positions", "accounts",
            ],
        ...
        Assert.Equal(7, curve.GetArrayLength());
        Assert.Equal(Scripted.Ms(2000).Value, curve[0].GetProperty("timestamp").GetInt64());
        Assert.Equal(999_999m, curve[0].GetProperty("equity").GetDecimal());
    ...
    public void Summary_prints_counts_and_the_per_currency_line_in_invariant_culture()
    {
        ...
        Assert.Contains(lines, l => l.Contains("max drawdown 9.00 (0.00%)  sharpe 1.31  sortino 2.29  profit factor 1.25  expectancy 0.67", StringComparison.Ordinal));
    }
    ...
    public void Write_all_creates_every_report_file_in_the_target_directory()
    {
        ...
        try
        {
            ...
            Assert.Equal(8, Lines(File.ReadAllText(Path.Combine(directory, "equity_USDT.csv"))).Length);
