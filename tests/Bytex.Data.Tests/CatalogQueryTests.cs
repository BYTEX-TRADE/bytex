...
public sealed class CatalogQueryTests : IDisposable
{
    ...
    [Fact(Skip = "BUG: the reader re-sorts with the unstable List.Sort, so records sharing one TsInit (every book snapshot) can come back in a different order than they were written")]
